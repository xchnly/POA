...
const ResignationRecapitulationPage: React.FC = () => {
    ...
    useEffect(() => {
        const applyFilters = () => {
            const filteredForms = allResignData.filter(form => {
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "ResignRecap");
        const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
        const data = new Blob([excelBuffer], { type: "application/octet-stream" });
        FileSaver.saveAs(data, "resignation_recapitulation.xlsx");
    };
            ...
            <div className="flex-1 overflow-auto">
                ...
                <main className="p-6">
                    <div className="bg-white rounded-xl shadow-md p-6 border border-green-100">
                        <h2 className="text-xl font-semibold text-gray-900 mb-4">Detailed Resignation Data</h2>
                        <div className="flex flex-col md:flex-row items-center justify-between mb-6 space-y-4 md:space-y-0 md:space-x-4">
                            ...
                            <button onClick={handleExport} className="px-6 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 transition w-full md:w-auto" disabled={resignData.length === 0}>Export to Excel</button>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                            {resignData.length > 0 ? (
                                <table className="min-w-full text-left table-auto border-collapse">
                                    <thead className="bg-gray-50">
                                        <tr className="border-b border-gray-200">
                                            ...
                                            <th className="py-3 px-4 text-sm font-semibold text-gray-600">Effective Date</th>
                                ...
                                <div className="text-center py-8 text-gray-500">
                                    <p>No resignation data found for the selected date range and department.</p>
                                </div>
                            )}
                        </div>
                    </div>
