...
const ReimburseRecapitulationPage: React.FC = () => {
    const [user, setUser] = useState<UserData | null>(null);
    const [allReimburseData, setAllReimburseData] = useState<ReimbursementRequest[]>([]);
    const [reimburseData, setReimburseData] = useState<ReimbursementRequest[]>([]);
    ...
    useEffect(() => {
        const applyFilters = () => {
            const filteredForms = allReimburseData.filter(form => {
            ...
            setReimburseData(filteredForms);
        };

        applyFilters();
    }, [allReimburseData, startDate, endDate, selectedDept, deptIdToName]);
    ...
    const handleExport = () => {
        ...
        reimburseData.forEach(form => {
            (form.items || []).forEach(item => { // Telah diperbaiki dari 'reimbursementItems' menjadi 'items'
                dataToExport.push({
                    ...
                    "Nama Bank": form.namaBank,
                    "Nomor Rekening": form.nomorRekening,
                    "Item": item.namaItem,
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "ReimburseRecap");
        const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
        const data = new Blob([excelBuffer], { type: "application/octet-stream" });
        FileSaver.saveAs(data, "reimbursement_recapitulation.xlsx");
    };
            ...
            <div className="flex-1 overflow-auto">
                ...
                <main className="p-6">
                    <div className="bg-white rounded-xl shadow-md p-6 border border-green-100">
                        <h2 className="text-xl font-semibold text-gray-900 mb-4">Detailed Reimbursement Data</h2>
                        <div className="flex flex-col md:flex-row items-center justify-between mb-6 space-y-4 md:space-y-0 md:space-x-4">
                            ...
                            <button onClick={handleExport} className="px-6 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 transition w-full md:w-auto" disabled={reimburseData.length === 0}>Export to Excel</button>
                        </div>
                        {/* Recapitulation Table */}
                        <div className="overflow-x-auto">
                            {reimburseData.length > 0 ? (
                                <table className="min-w-full text-left table-auto border-collapse">
                                    <thead className="bg-gray-50">
                                        <tr className="border-b border-gray-200">
                                            ...
                                            <th className="py-3 px-4 text-sm font-semibold text-gray-600">Bank Name</th>
                                            <th className="py-3 px-4 text-sm font-semibold text-gray-600">Account No.</th>
                                            ...
                                            (form.items || []).map((item, index) => { // Telah diperbaiki dari 'reimbursementItems' menjadi 'items'
                                                ...
                                                return (
                                                    <tr key={`${form.id}-${index}`} className="border-b border-gray-100 hover:bg-green-50">
                                                        ...
                                                        <td className="py-3 px-4 text-sm text-gray-600">{form.namaBank}</td>
                                                        <td className="py-3 px-4 text-sm text-gray-600">{form.nomorRekening}</td>
                                                        ...
                                                        <td className="py-3 px-4 text-sm text-gray-600">Rp.{item.harga.toLocaleString('id-ID')},00</td>
                                ...
                                <div className="text-center py-8 text-gray-500">
                                    <p>No reimbursement data found for the selected date range and department.</p>
                                </div>
                            )}
                        </div>
                    </div>
...
export default ReimburseRecapitulationPage;