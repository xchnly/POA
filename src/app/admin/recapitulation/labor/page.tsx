...
const LaborRecapitulationPage: React.FC = () => {
    const [user, setUser] = useState<UserData | null>(null);
    const [allLaborData, setAllLaborData] = useState<LaborRequest[]>([]);
    const [laborData, setLaborData] = useState<LaborRequest[]>([]);
                ...
                try {
                    ...
                    if (!userDoc.empty) {
                        ...
                        const processedForms = allForms.map(form => {
                            const deptName = idToNameMap.get(form.deptId as string) || form.deptId;
                            return { ...form, deptName: deptName };
                        });

                        setAllLaborData(processedForms);
    ...
    useEffect(() => {
        const applyFilters = () => {
            const filteredForms = allLaborData.filter(form => {
            ...
            setLaborData(filteredForms);
        };

        applyFilters();
    }, [allLaborData, startDate, endDate, selectedDept, deptIdToName]);

    const handleExport = () => {
        const dataToExport: any[] = laborData.map((form, index) => ({
            "No": index + 1,
            "Form ID": form.id,
            "Requester Name": form.requesterName,
            "Date Submitted": safeToDate(form.createdAt)?.toLocaleDateString('en-US'),
            "Required Date": form.tanggalDiperlukan,
            "Department": deptIdToName.get(form.deptId as string),
            "Position": form.posisi,
            "Required People": form.jumlahOrang,
            "Gender": form.gender.join(", "),
            "Age Limit": form.umurMaksimal,
            "Main Task": form.tugasUtama,
            "Required Language": form.bahasaDiperlukan.join(", "),
            "Other Requirements": form.persyaratanLain,
            "Reason": form.alasanMemerlukan,
            "Final Status": formatStatusText(getFinalStatus(form.approvalFlow)),
        }));

        const worksheet = XLSX.utils.json_to_sheet(dataToExport);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "LaborRecap");
        const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
        const data = new Blob([excelBuffer], { type: "application/octet-stream" });
        FileSaver.saveAs(data, "labor_recapitulation.xlsx");
    };
            ...
            <div className="flex-1 overflow-auto">
                ...
                <header className="bg-white shadow-sm border-b border-green-100">
                    <div className="flex items-center justify-between p-4">
                        <div className="flex items-center">
                            ...
                            <div>
                                <h1 className="text-xl md:text-2xl font-bold text-gray-900">Labor Recapitulation</h1>
                                <p className="text-xs md:text-sm text-gray-500">View and export all labor request data</p>
                            </div>
                        </div>
                ...
                <main className="p-6">
                    <div className="bg-white rounded-xl shadow-md p-6 border border-green-100">
                        <h2 className="text-xl font-semibold text-gray-900 mb-4">Detailed Labor Data</h2>
                        <div className="flex flex-col md:flex-row items-center justify-between mb-6 space-y-4 md:space-y-0 md:space-x-4">
                            ...
                            <button onClick={handleExport} className="px-6 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 transition w-full md:w-auto" disabled={laborData.length === 0}>Export to Excel</button>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                                ...
                                <table className="min-w-full text-left table-auto border-collapse">
                                    <thead className="bg-gray-50">
                                        <tr className="border-b border-gray-200">
                                            ...
                                            <th className="py-3 px-4 text-sm font-semibold text-gray-600">Required Date</th>
                                            <th className="py-3 px-4 text-sm font-semibold text-gray-600">Department</th>
                                            <th className="py-3 px-4 text-sm font-semibold text-gray-600">Position</th>
                                            <th className="py-3 px-4 text-sm font-semibold text-gray-600">Required People</th>
                                            <th className="py-3 px-4 text-sm font-semibold text-gray-600">Reason</th>
                                            <th className="py-3 px-4 text-sm font-semibold text-gray-600">Status</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {laborData.map((form, index) => (
                                            <tr key={form.id} className="border-b border-gray-100 hover:bg-green-50">
                                                ...
                                                <td className="py-3 px-4 text-sm text-gray-600">{form.tanggalDiperlukan}</td>
                                                <td className="py-3 px-4 text-sm text-gray-600">{deptIdToName.get(form.deptId as string)}</td>
                                                <td className="py-3 px-4 text-sm text-gray-600">{form.posisi}</td>
                                                <td className="py-3 px-4 text-sm text-gray-600">{form.jumlahOrang}</td>
                                                <td className="py-3 px-4 text-sm text-gray-600">{form.alasanMemerlukan}</td>
                            ...
                            ) : (<div className="text-center py-8 text-gray-500"><p>No labor request data found for the selected date range and department.</p></div>)}
                        </div>
                    </div>
                </main>
            </div>
...
export default LaborRecapitulationPage;